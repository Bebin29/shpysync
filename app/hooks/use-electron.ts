...
export function useElectron() {
  ...
  const csv = {
    ...
    preview: useCallback(
      async (config: { filePath: string; mapping: ColumnMapping; maxRows?: number }) => {
        if (!isAvailable) {
          throw new Error("Electron API nicht verfügbar");
        }
        return window.electron.csv.preview(config) as Promise<
          import("../../electron/types/ipc").CsvPreviewResponse
        >;
      },
      [isAvailable]
    ),
