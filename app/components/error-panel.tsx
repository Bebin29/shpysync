 ...
 */
export function ErrorPanel({ error, onDismiss, showDetails = false, className }: ErrorPanelProps) {
  if (!error) {
    return null;
  }
  ...
  const getVariant = (): "default" | "destructive" => {
    return error.severity === "error" || error.severity === "fatal" ? "destructive" : "default";
  };
  ...
  return (
    <Alert variant={getVariant()} className={`${getSeverityColor()} ${className || ""}`}>
      {getIcon()}
      <AlertTitle className="flex items-center justify-between">
        ...
        {onDismiss && (
          <Button variant="ghost" size="sm" onClick={onDismiss} className="h-auto p-0 ml-2">
            ×
          </Button>
        )}
      </AlertTitle>
          ...
          {showDetails && error.details !== undefined && error.details !== null && (
            <details className="mt-2 text-xs opacity-75">
              ...
              <p className="mt-2 text-xs italic">
                Hinweis: Technische Details werden nur im Development-Modus angezeigt.
              </p>
            </details>
          )}
