...
export function UpdateSection() {
  ...
  if (!status) {
    return (
      <Button variant="outline" size="sm" onClick={handleCheckForUpdates} disabled={false}>
  ...
  if (status.error) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>Update-Fehler: {status.error}</AlertDescription>
      </Alert>
    );
  }
    ...
    return (
      <Alert className="bg-green-50 border-green-200">
        ...
        <AlertDescription className="flex items-center justify-between">
          <div>
            <p className="font-semibold text-green-900">Update bereit zur Installation</p>
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            {status.info?.releaseNotes && (
              <p className="text-sm text-blue-700 mb-3">{status.info.releaseNotes}</p>
            )}

            {status.downloading && (
              <div className="space-y-2 mt-3">
                <Progress value={status.progress} />
                <p className="text-sm text-blue-700">Download: {Math.round(status.progress)}%</p>
              </div>
            )}

            {!status.downloading && (
              <Button onClick={handleDownload} className="w-full mt-2" size="sm">
  ...
  return (
    <div className="space-y-2">
      ...
      {status.checking && (
        <p className="text-sm text-muted-foreground">Prüfe auf verfügbare Updates...</p>
      )}
    </div>
  );
}
