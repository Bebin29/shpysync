      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Überprüfe die geplanten Updates vor der Ausführung</CardDescription>
          </div>
          {onExport && (
      ...
      <CardContent className="space-y-4">
        {hasUnmatched && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              {unmatchedCount} Zeile{unmatchedCount !== 1 ? "n" : ""} konnte
              {unmatchedCount === 1 ? "" : "n"} nicht gematcht werden.
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "all" | "unmatched")}>
            <TabsList>
              ...
              {hasUnmatched && (
                <TabsTrigger value="unmatched">Nicht gematcht ({unmatchedCount})</TabsTrigger>
              )}
            </TabsList>
          </Tabs>
              ...
              <TableBody>
                {filteredRows.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>
                      <div>
                        {row.productTitle && <div className="font-medium">{row.productTitle}</div>}
                        {row.variantTitle && (
        ...
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          <div>
            Zeige {filteredRows.length} von{" "}
            {activeTab === "unmatched" ? unmatchedCount : rows.length} Zeile
