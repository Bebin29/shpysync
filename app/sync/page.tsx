...
export default function SyncPage() {
  ...
  const [operationTypeFilter, setOperationTypeFilter] = useState<"all" | "price" | "inventory">(
    "all"
  );
  const [statusFilter, setStatusFilter] = useState<
    "all" | "planned" | "success" | "failed" | "skipped"
  >("all");
  ...
  const {
    setCsvFilePath: setStoreCsvPath,
    setStep,
    addLog,
    setProgress,
    setCurrentAction,
  ...
  useEffect(() => {
    const loadDefaultPath = async () => {
      try {
        ...
        const defaultPath =
          (config as { defaultDbfPath?: string; defaultCsvPath?: string }).defaultDbfPath ||
  ...
  const handleLoadPreview = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        const unmatchedRows = result.data.unmatchedRows.map(
          (
            row: { rowNumber: number; sku: string; name: string; price?: string; stock?: number },
            index: number
          ) => ({
  ...
  const handleTestSync = useCallback(async () => {
    ...
    const selectedOperation = plannedOperations.find((op) => op.id === selectedTestOperationId);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">CSV-Datei hochladen und mit Shopify synchronisieren</p>
      </div>
      ...
      <div className="flex items-center justify-center gap-2">
        {[1, 2, 3, 4].map((step) => (
          <div key={step} className="flex items-center">
            ...
            {step < 4 && (
              <div className={`h-1 w-16 ${currentStep > step ? "bg-green-500" : "bg-gray-300"}`} />
            )}
          </div>
        ))}
      </div>
      ...
      <div className="flex justify-between">
        ...
        {currentStep < 4 && (
          <Button onClick={handleNext} disabled={!canProceed || isLoadingPreview || isSyncRunning}>
            {isLoadingPreview ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                Lädt...
