...
function migrateOldConfig(config: unknown): AppConfig | null {
  ...
  const appConfig: AppConfig = {
    ...
    autoSync: isObject(config.autoSync)
      ? (config.autoSync as AppConfig["autoSync"])
      : { enabled: false },
    ...
    if (typeof shop.accessToken === "string" && !shop.accessTokenRef) {
      ...
      try {
        ...
        (store as unknown as { set: (key: string, value: unknown) => void }).set(
          ...
          appConfig.defaultColumnMapping
        );
...
export function getShopConfig(): ShopConfig | null {
  const stored = (store as unknown as { get: (key: string) => unknown }).get(
    "shop"
  ) as ShopConfigStored | null;
  if (!stored) {
    return null;
  }
...
export function setShopConfig(shopConfig: ShopConfig | null): void {
  ...
  try {
    ...
    throw WawiError.configError(
      "CONFIG_INVALID",
      `Validierungsfehler: ${error instanceof Error ? error.message : String(error)}`
    );
  }

  // Prüfe ob bereits eine Config existiert
  const existing = (store as unknown as { get: (key: string) => unknown }).get(
    "shop"
  ) as ShopConfigStored | null;
  let tokenRef: string;
...
export function getDefaultColumnMapping(): ColumnMapping | null {
  return (
    ((store as unknown as { get: (key: string) => unknown }).get(
      "defaultColumnMapping"
    ) as ColumnMapping | null) ?? null
  );
}
