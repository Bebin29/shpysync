...
import type { SyncHistoryEntry, HistoryStats } from "../types/ipc.js";
...
class SyncHistoryService {
  ...
  private loadHistory(): SyncHistoryEntry[] {
    ...
    try {
      ...
      this.logger.warn("history", "Fehler beim Laden der Historie, erstelle Backup", {
  ...
  private saveHistory(history: SyncHistoryEntry[]): void {
    ...
    try {
      ...
      this.logger.error("history", "Fehler beim Speichern der Historie", {
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
    }
  ...
  saveSyncHistory(entry: SyncHistoryEntry): void {
    ...
    this.logger.info("history", "Sync-Eintrag in Historie gespeichert", {
      id: entry.id,
