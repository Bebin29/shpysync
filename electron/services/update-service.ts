...
import type { UpdateStatus } from "../types/ipc.js";
...
export class UpdateService {
  ...
  private setupAutoUpdater(): void {
    ...
    if (githubToken) {
      this.logger.info(
        "update",
        "GitHub Token gefunden - Updates für private Repositories aktiviert"
      );
    } else {
      this.logger.info(
        "update",
        "Kein GitHub Token gesetzt - Updates nur für öffentliche Repositories"
      );
    }
        ...
        releaseNotesStr = releaseNotes
          .map((note) => {
            ...
            if (note && typeof note === "object") {
              return (
                (note as { name?: string; note?: string; text?: string }).name ||
  ...
  startAutoCheck(intervalHours: number = 24): void {
    ...
    this.logger.info(
      "update",
      `Automatische Update-Prüfung alle ${intervalHours} Stunden aktiviert`
    );
  }
