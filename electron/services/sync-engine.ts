...
import type { CsvRow, Product, Variant } from "../../core/domain/types.js";
import {
  extractRowValues,
  convertToCsvRows,
...
// Diese Funktionen sind für zukünftige Verwendung reserviert
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function calculateTotalWorkUnits(_workUnits: WorkUnits): number {
  return 0; // Placeholder
}

// eslint-disable-next-line @typescript-eslint/no-unused-vars
function calculateProgress(_completed: number, _total: number): number {
  return 0; // Placeholder
}
...
export class SyncEngine {
  ...
  async startSync(config: SyncStartConfig): Promise<SyncResult> {
    ...
    try {
      ...
      this.logger.info("csv", `${csvRows.length} Zeilen aus ${fileTypeLabel} geladen.`, {
      ...
      this.logger.info("shopify", `${products.length} Produkte mit Varianten geladen.`, {
      ...
      plannedOperations = this.generatePlannedOperations(updateResult, products, syncMode);

      totalPlanned = plannedOperations.length;

      this.logger.info(
        "matching",
        `${updateResult.mappedRows.length} Zeilen gematcht, ${updateResult.unmatchedRows.length} nicht gematcht.`,
        {
      ...
      this.logger.info(
        "system",
        `${updateResult.priceUpdates.length} Preis-Updates geplant, ${updateResult.inventoryUpdates.length} Inventory-Updates geplant.`,
        {
      ...
      try {
        ...
        this.logger.warn("history", "Fehler beim Speichern der Historie", {
          error: error instanceof Error ? error.message : String(error),
        });
      }
   ...
   */
  private async parseFileWithMapping(filePath: string, mapping: ColumnMapping): Promise<CsvRow[]> {
  ...
  async generatePreview(config: SyncPreviewRequest): Promise<{
    ...
    // Schritt 7: Vorschau generieren
    const plannedOperations = this.generatePlannedOperations(updateResult, products, syncMode);
  ...
  private async executeUpdates(
    ...
    const inventoryItemToCsvRowMap = new Map<
      string,
      { productId: string; variant: Variant; csvRow: CsvRow }
    >();
        ...
        try {
          ...
          if (result.success && result.userErrors.length === 0) {
            ...
            for (const update of updates) {
              const variantData = variantMap.get(update.variantId);
              if (variantData && variantData.csvRow) {
                const error = result.userErrors.find((e) => e.field?.includes(update.variantId));
                operations.push({
                  type: "price",
                  csvRow: {
                    rowNumber: variantData.csvRow.rowNumber,
          ...
          this.logger.error(
            "price",
            `Fehler beim Aktualisieren der Preise für Produkt ${productId}: ${errorMessage}`,
            {
              productId,
              error: errorMessage,
        ...
        try {
          ...
          if (result.success && result.userErrors.length === 0) {
            ...
            for (const update of batch) {
              const variantData = inventoryItemToCsvRowMap.get(update.inventoryItemId);
              if (variantData) {
                const error = result.userErrors.find((e) =>
                  e.field?.includes(update.inventoryItemId)
                );
                operations.push({
                  type: "inventory",
                  csvRow: {
          ...
          this.logger.error(
            "inventory",
            `Fehler beim Aktualisieren der Bestände: ${errorMessage}`,
            {
              error: errorMessage,
  ...
  async testSync(
    ...
    const testOperation = plannedOperations.find(
      (op) => op.type === "inventory" && typeof op.newValue === "number" && op.newValue > 0
    );
    ...
    this.logger.info(
      "system",
      `Test-Sync: Artikel "${testOperation.productTitle || testOperation.variantTitle || testOperation.sku}" wird getestet (Bestand: ${testOperation.oldValue} → ${testOperation.newValue})`,
      {
    ...
    try {
      ...
      if (result.success && result.userErrors.length === 0) {
        ...
        this.logger.info(
          "inventory",
          `Test-Sync erfolgreich! Bestand wurde von ${testOperation.oldValue} auf ${testOperation.newValue} aktualisiert.`,
          {
            oldValue: testOperation.oldValue,
            newValue: testOperation.newValue,
          }
