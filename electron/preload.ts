...
const electronAPI = {
  ...
  // Sync-Funktionen
  sync: {
    preview: (config: SyncPreviewRequest) =>
      ipcRenderer.invoke("sync:preview", config) as Promise<SyncPreviewResponse>,
    start: (config: SyncStartConfig) =>
      ipcRenderer.invoke("sync:start", config) as Promise<{
        success: boolean;
        message?: string;
        error?: string;
      }>,
    test: (config: SyncTestRequest) =>
      ipcRenderer.invoke("sync:test", config) as Promise<{
        success: boolean;
        message?: string;
        error?: string;
      }>,
    cancel: () =>
      ipcRenderer.invoke("sync:cancel") as Promise<{
        success: boolean;
        message?: string;
        error?: string;
      }>,
  ...
  config: {
    ...
    setColumnMapping: (mapping: unknown) =>
      ipcRenderer.invoke("config:set-column-mapping", mapping),
    testConnection: (shopConfig: unknown) =>
      ipcRenderer.invoke("config:test-connection", shopConfig),
  ...
  csv: {
    ...
    preview: (config: { filePath: string; mapping: unknown; maxRows?: number }) =>
  ...
  autoSync: {
    ...
    start: () =>
      ipcRenderer.invoke("autoSync:start") as Promise<{ success: boolean; error?: string }>,
    stop: () =>
      ipcRenderer.invoke("autoSync:stop") as Promise<{ success: boolean; error?: string }>,
    getConfig: () => ipcRenderer.invoke("autoSync:getConfig") as Promise<AppConfig["autoSync"]>,
    setConfig: (config: AppConfig["autoSync"]) =>
      ipcRenderer.invoke("autoSync:setConfig", config) as Promise<{
        success: boolean;
        error?: string;
      }>,
    testSync: (csvPath: string) =>
      ipcRenderer.invoke("autoSync:testSync", csvPath) as Promise<{
        success: boolean;
        error?: string;
      }>,
  },
  ...
  dashboard: {
    ...
    getHistory: (limit?: number) =>
      ipcRenderer.invoke("dashboard:get-history", limit) as Promise<SyncHistoryEntry[]>,
  },
  ...
  update: {
    ...
    download: () =>
      ipcRenderer.invoke("update:download") as Promise<{ success: boolean; error?: string }>,
    install: () =>
    ...
    onError: (callback: (error: { message: string }) => void) => {
      ipcRenderer.on("update:update-error", (_event, error: { message: string }) =>
        callback(error)
      );
    },
    removeAllListeners: (channel: string) => {
...
try {
  ...
  console.log(
    "[Preload] Dashboard Keys:",
    electronAPI.dashboard ? Object.keys(electronAPI.dashboard) : "N/A"
  );
} catch (error) {
