...
export class WawiError extends Error {
   ...
   *
   * WICHTIG: Diese Methode sollte nur für Server-seitige Logs verwendet werden.
   * Stack-Traces und sensible Daten werden hier nicht gefiltert.
   */
  ...
  /**
   * Prüft, ob der Fehler sensible Daten enthält, die nicht an den User weitergegeben werden sollten.
   */
  public containsSensitiveData(): boolean {
    ...
    const sensitiveKeys = ["token", "password", "secret", "key", "accessToken", "apiKey"];
  ...
  public static fromError(error: unknown, code: ErrorCode = "INTERNAL_UNEXPECTED"): WawiError {
    ...
    return new WawiError(code, `Unbekannter Fehler: ${String(error)}`, "error", {
      originalError: error,
    });
  }
  ...
  public static shopifyError(
    ...
    const severity: ErrorSeverity =
      code === "SHOPIFY_RATE_LIMIT" || code === "SHOPIFY_SERVER_ERROR" ? "warning" : "error";
