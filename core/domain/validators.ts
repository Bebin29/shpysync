 ...
 */
export function validateColumnMapping(mapping: ColumnMapping, headers: string[]): void {
  if (!mapping) {
...
export function validateAccessToken(token: string, strict: boolean = false): void {
  ...
  if (!strict) {
    if (trimmedToken.length < 10) {
      throw WawiError.configError("CONFIG_TOKEN_INVALID", "Access-Token ist zu kurz", {
 ...
 */
export function validateShopConfig(
  config: {
    shopUrl: string;
    accessToken: string;
    locationId: string;
  ...
  strictTokenValidation: boolean = false
): void {
  if (!config) {
