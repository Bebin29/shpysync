...
export function parseCsv(filePath, delimiter = ";") {
 ...
 */
export function extractRowValues(row, columnMapping, headers) {
    try {
 ...
 */
export function convertToCsvRows(extractedRows) {
    const csvRows = [];
 ...
 */
export async function parseCsvStream(filePath, delimiter = ";") {
    // Encoding-Erkennung
 ...
 */
export async function parseCsvPreview(filePath, maxRows = 200, delimiter = ";") {
    const rows = [];
    let headers = null;
    let encoding = "utf-8";
    let rowCount = 0;
