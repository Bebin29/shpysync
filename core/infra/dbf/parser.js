...
exports.parseDbfStream = parseDbfStream;
exports.parseDbfPreview = parseDbfPreview;
 ...
 */
function readNullTerminatedString(buffer, offset, maxLength, encoding) {
  ...
  return iconv_lite_1.default.decode(bytes, encoding).trim();
}
/**
 ...
 */
function normalizeEncodingName(codepageName) {
 ...
 */
function detectDbfEncoding(header, buffer, fields) {
        ...
        try {
          ...
          for (let i = 0; i < fieldsToTest; i++) {
            ...
            if (field.type === "C") {
              ...
              const decoded = iconv_lite_1.default.decode(fieldData, enc).trim();
 ...
 */
function parseDbfFields(buffer, headerLength, encoding) {
  const fields = [];
 ...
 */
function convertDbfFieldValue(buffer, offset, field, encoding) {
  const fieldData = buffer.slice(offset, offset + field.length);
  switch (field.type) {
    case "C": // Character
      return iconv_lite_1.default.decode(fieldData, encoding).trim();
    ...
    case "N": // Numeric (als ASCII-String gespeichert)
      const numericStr = iconv_lite_1.default.decode(fieldData, encoding).trim();
    ...
    case "D": // Date (YYYYMMDD als 8 Bytes ASCII)
      const dateStr = iconv_lite_1.default.decode(fieldData, encoding).trim();
...
function parseDbf(filePath) {
  // Validiere Datei
  (0, validators_js_1.validateDbfFile)(filePath);
  ...
  // Validiere Header
  (0, validators_js_1.validateDbfHeaders)(headers);
  // Records parsen
  const rows = [];
 ...
 */
async function parseDbfStream(filePath) {
  // Validiere Datei
  (0, validators_js_1.validateDbfFile)(filePath);
 ...
 */
async function parseDbfPreview(filePath, maxRows = 200) {
  // Validiere Datei
  (0, validators_js_1.validateDbfFile)(filePath);
  const rows = [];
  let headers = [];
